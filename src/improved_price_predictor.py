...
class AdvancedPricePredictor:
    ...
    def calculate_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Target: future price change (next 24 hours)
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        feature_cols = ['returns', 'log_returns', 'sma_7', 'sma_14', 'sma_30', 
                       'ema_7', 'ema_14', 'rsi', 'macd', 'macd_signal', 'macd_diff',
                       'bb_upper', 'bb_lower', 'bb_width', 'volatility',
                       'volume_ratio', 'price_volume_corr', 'price_position',
                       'roc_7', 'roc_14']
    ...
    def train_ensemble_model(self, coin_id: str, days: int = 90):
        ...
        try:
            ...
            joblib.dump({'rf': model_predictions['rf']['model'], 
                        'gb': model_predictions['gb']['model']}, model_path)
            ...
            return True, f"Model trained. MAE: {ensemble_mae:.4f}, RMSE: {ensemble_rmse:.4f}"
            
        except Exception as e:
            return False, f"Training error: {str(e)}"
    
    def predict_price(self, coin_id: str, current_price: float) -> Dict:
        """Predict price change for next 24 hours"""
        try:
            ...
            if coin_id not in self.models:
                ...
                if not os.path.exists(model_path):
                    ...
                    success, message = self.train_ensemble_model(coin_id)
                    if not success:
            ...
            df = self.api.get_coin_history(coin_id, days=30)
            ...
            confidence = max(0, 100 - recent_volatility * 2)
            confidence = min(confidence, 95)  # Cap at 95%
            
            # Generate insights
            insights = self._generate_insights(df, predicted_change)
    ...
    def _generate_insights(self, df: pd.DataFrame, predicted_change: float) -> list:
        """Generate trading insights based on technical analysis"""
        insights = []
        ...
        if abs(recent_trend) > 3:
            ...
            insights.append(f"Strong {trend_direction} trend ({abs(recent_trend):.1f}% in 5 days)")
        ...
        if abs(predicted_change) > 3:
            direction = "significant increase" if predicted_change > 0 else "significant decrease"
            insights.append(f"Model predicts {direction} ({abs(predicted_change):.1f}%)")
